---
id: devices/TED-0002
title: "DynaVap M7 Mechanical Thermal Extractor"
parent: devices
status: published
tags: ["device", "portable", "analog", "direct-draw", "dynavap", "torch"]
relations: [relates_to=manufacturers/TMFR-0002]
summary: Battery-free stainless steel mechanical thermal extraction device from DynaVap.
---

# DynaVap M7 (TED-0002)

{{include includes/manufacturer-claim-note.md}}

## Technical Specifications

| Property | Specification |
| --- | --- |
| Manufacturer | [DynaVap LLC](../manufacturers/dynavap.md) |
| Release Year | 2024 (with M7 XL) [^1] |
| Heating Method | Conduction-dominant thermal mass storage with convection draw |
| Operation Mode | On-Demand (analog, user-controlled) |
| Temperature Control | Bimetallic disc Captive Cap with an audible "click"; **not** digital. DynaVap states the Captive Cap signals at ~240 °C ± 10 °C (~230–250 °C); the Low-Temp Cap signals at ~215 °C ± 10 °C (~205–220 °C) [^2] |
| Bowl Capacity | ~0.1 g (Adjust-a-Bowl reduces to ~0.05 g) [^3] |
| Airpath Materials | Stainless steel stem, tip, and condenser (per manufacturer). "Medical grade stainless steel" is DynaVap marketing language; DynaVap does not state the 316 grade on the M7 pages [^4] |
| Power Source | External thermal input (butane torch or electromagnetic induction heater) |
| Heat-Up Time | ~5–10 seconds (induction) / ~10–15 seconds (flame) — per DynaVap marketing |
| Compatible Media | Dry botanical flower; concentrates via the [DynaCoil](https://www.dynavap.com/products/dynacoil) concentrate adapter |

## Warranty & Service

- DynaVap offers a limited lifetime warranty on functional metal/components and on replacement parts; caps, CCDs, O-rings, and accessories are excluded [^5].
- Device is fully user-serviceable and disassembles without tools.

## Cleaning & Maintenance Protocol

1. **Disassembly**: Remove Captive Cap, tip, condenser, and fluoroelastomer o-rings.
2. **Cleaning**: Soak stainless steel stem, tip, and condenser in isopropyl alcohol. **Do not soak the Captive Cap** in alcohol or water; DynaVap recommends cleaning the cap with a swab, not soaking [^6].
3. **Reassembly**: Lubricate o-rings with DynaWax or hemp oil and reassemble condenser into stem body.

## Accessories & Consumables

- Wand Induction Heater
- Replacement Stainless Steel CCD Screens
- High-Temp O-Ring Kits
- DynaCoil (concentrates)

## Sources

[^1]: DynaVap, "M 7" product history and 2024 lineup announcement. https://www.dynavap.eu/blogs/news/m-woodwynd-vong-b-the-2024-dynavap-line-up
[^2]: DynaVap FAQ / support: Captive Cap clicks at approximately 240 °C ± 10 °C; Low-Temp Cap approximately 215 °C ± 10 °C. https://www.dynavap.com/pages/faq
[^3]: DynaVap, "Stainless Steel Tip: VonG X" page, which quantifies the Adjust-a-Bowl as 0.1 g full or 0.05 g microdose. https://www.dynavap.com/products/stainless-steel-tip-vong-x
[^4]: DynaVap product pages use "medical grade stainless steel" (e.g., "How to use" and Cap pages). The specific "316" grade is stated by DynaVap for some components, not stated for the M7 pages.
[^5]: DynaVap Device Warranty (limited lifetime warranty on metal components; caps, CCDs, O-rings excluded). https://dynavap.freshdesk.com/support/solutions/articles/1510000
[^6]: DynaVap cleaning guide: do not soak the Cap. https://dynavap.freshdesk.com/support/solutions/articles/151000083483-how-to-clean-your-device