...
* `./scripts/ted-publish.sh`: Exports HTML, IR, working RAG, semantically named upload packs, the complete RAG corpus, Context, sitemap, and `llms.txt` artifacts.
...
scripts/ted-publish.sh     # HTML, IR, named RAG uploads, complete RAG, Context, and llms
bin/validate_graph.sh      # Graph integrity and publication gate
```

Generated outputs under `dist/`, `publish/`, `site/`, `.tools/`, and local compiler binaries (`bin/boris*`) are build artifacts and must not be committed to git. The publish directory contains `rag/` (canonical Boris working-context packs), `rag-bundle/` (semantically named `.md` files ready for upload), and `rag-complete/` (the explicit full-corpus tree). See [`docs/rag-upload.md`](docs/rag-upload.md) for the upload workflow. `metadata/id-map.jsonl` is the one deliberate exception: it is generated by `scripts/ted_ids.py --write` and kept tracked as the identity migration record.
