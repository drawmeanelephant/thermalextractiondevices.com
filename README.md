...
* `./scripts/ted-publish.sh`: Exports HTML, IR, raw and resolved working RAG,
...
scripts/ted-publish.sh     # HTML, IR, raw/resolved RAG uploads, complete RAG, Context, and llms
bin/validate_graph.sh      # Graph integrity and publication gate
```

Generated outputs under `dist/`, `publish/`, `site/`, `.tools/`, and local compiler binaries (`bin/boris*`) are build artifacts and must not be committed to git. The publish directory contains `rag/` (canonical raw Boris working-context packs), `rag-bundle/` (semantically named raw packs), `rag-resolved/` (derived packs with include bodies expanded), `rag-resolved-bundle/` (semantically named resolved packs ready for upload), and `rag-complete/` (the explicit raw full-corpus tree). See [`docs/rag-upload.md`](docs/rag-upload.md) for the upload workflow. `metadata/id-map.jsonl` is the one deliberate exception: it is generated by `scripts/ted_ids.py --write` and kept tracked as the identity migration record.
